...
def choose_ship(sigma,Phi,B,choose_berth):
    if len(Phi) == 0: # 只执行一次
        ...
        for vessel in sigma:
            ...
            for berth in B:
                dic_vessel_ect[(vessel,berth)] = r_j['r'+str(vessel)]+sum([p_a['p'+str(temp)] for temp in range(alpha,y_j)])+p_a_k['p'+str(y_j)+str(berth)]
        ...
        for vessel in sigma:
            for berth in B:
                ...
                for i in choose_berth: 
                    if berth == i[1]: # 判断是否是同一泊位,是的话再执行以下条件
                ...
                b = max_ship_values
                dic_vessel_ect[(vessel,berth)] =max(a,b) +p_a_k['p'+str(y_j)+str(berth)]
...
def calculate(sheet_name,problem_num,chsegs,berth_num,ship_num,filename):
    ...
    for i in T_berth:
        for j in B:
            ...
            index_berth+=1
    # print('p_a_k',p_a_k)
    '''p_a_k {'p30': 110.24, 'p31': 106.682, 'p32': 102.255, 'p80': 66.8602, 'p81': 59.0591, 'p82': 64.9694, 'p130': 51.4725, 'p131': 95.7229, 'p132': 93.6057, 'p180': 103.555, 'p181': 58.8519, 'p182': 63.3849, 'p230': 116.552, 'p231': 87.7315, 'p232': 73.7655, 'p280': 53.8197, 'p281': 54.7083, 'p282': 80.9662}'''
    ...
    # print('p_a:',p_a)
    for i in T_berth:
        sum_z+=(t_a['t'+str(i+1)]-t_a['t'+str(i)]-min([p_a_k['p'+str(i)+str(j)] for j in B]))
        # print(t_a['t'+str(i+1)],t_a['t'+str(i)],p_a['p'+str(i)])
