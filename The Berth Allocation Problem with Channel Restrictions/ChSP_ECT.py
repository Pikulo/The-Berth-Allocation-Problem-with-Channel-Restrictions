...
# 返回选择的船舶
def choose_ship(sigma,Phi):
    if len(Phi) == 0: # 只执行一次
        dic_vessel_ect = {}
        for vessel in sigma:
            alpha = T_dict['T_'+str(vessel+1)][0]
            y_j = T_dict['T_'+str(vessel+1)][(ship_turn//2)]
            dic_vessel_ect[vessel] = r_j['r'+str(vessel)]+sum([p_a['p'+str(temp)] for temp in range(alpha,y_j)])+p_a['p'+str(y_j)]
        # dic_vessel_ect = dict(sorted(dic_vessel_ect.items(), key=lambda x: x[1]))
        min_ectship = min(dic_vessel_ect.items(), key=lambda x: x[1])[0]
        return min_ectship
    else:
        dic_vessel_ect = {}
        for vessel in sigma:
            max_right = []
            for i in Phi: 
                if berths_al[vessel] == berths_al[i]: # 判断是否是同一泊位,是的话再执行以下条件
                    y_j = T_dict['T_'+str(i+1)][(ship_turn//2)]
                    max_right.append(t_a['t'+str(y_j)]+p_a['p'+str(y_j)]+delta_3)
                else:
                    max_right.append(0.0)
            max_right.sort()
            max_ship_values = max_right[-1] # 取最大值
            alpha = T_dict['T_'+str(vessel+1)][0]
            y_j = T_dict['T_'+str(vessel+1)][(ship_turn//2)]
            a = r_j['r'+str(vessel)]+sum([p_a['p'+str(temp)] for temp in range(alpha,y_j)])
            b = max_ship_values
            dic_vessel_ect[vessel] =max(a,b) +p_a['p'+str(y_j)]
        min_ectship = min(dic_vessel_ect.items(), key=lambda x: x[1])[0]
        return min_ectship

def calculate(sheet_name,problem_num,chsegs,ship_num,filename):
    ''' sheet_name:表名,
    ...
    global berths_al
    berths_al = [int(i) for i in df_berth.values] # 船舶0到n的泊位
    ...
    sigma = [i for i in range(ship_num)] # ETC下与初始顺序无关,而是取决于船舶到达时间以及在泊位停留时间。
    Phi = [] # ships scheduled at the current point
    sigma_copy = copy.deepcopy(sigma)

    global t_a
    t_a = {}
    for i in range((chsegs*2+1)*ship_num):
        t_a['t'+str(i+1)] = 0.0
    # print(t_a)
    ...
    for i in range(len(sigma)):
        ...
        if len(Phi) == 0:
            ...
            for ship in Phi:
                ...
                '''outbound_a = outbound_a[::-1]这个式子是错误的'''
